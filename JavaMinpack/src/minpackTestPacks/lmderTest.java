...
//	    ! originally from file22
	    ...
	    static int[] info_original = new int[] {3,3,1,1,1,1,4,2,2,2,2,2,4,4,4,1,1,1,1,1,1,
	                                                        1,1,5,2,5,1,1,1,3,1,3,3,3,2,2,1,1,1,1,4,1,
	                                                        1,1,2,1,2,2,2,2,2,1,1};// !! original `info` from the original minpack

	    static int i, ic =0, info, k, ldfjac, lwa, m, n, nFev, nJev, nProb, ntries, icase, iunit;
...
//	    int nwrite = output_unit //! logical output unit
	    ...
	    for( icase = 0;icase<ncases+1;icase++) {
	        ...
	        if (icase == ncases+1) {
	            System.out.println("(A,I3,A/)" + "1SUMMARY OF " + ic + " CALLS TO LMDER1");
...
//	            if (allocated(iwa))  deallocate(iwa);  allocate(iwa(n))
	            ...
	            for(k=0;k<ntries;k++){
	                ...
	                nFev = 0;
	                nJev = 0;
	                Minpack.lmder1(fcn, m, n, x, fVec, fJac, ldfjac, tol, info, iwa, wa, lwa);
	                ssqfcn(m, n, x, fVec, nProb);
...
//
	    double[] diff = new double[x.length], absdiff = new double[x.length], reldiff = new double[x.length];
	    if (info_original[ic-1]<5) {   // ! ignore any where the original minpack failed
	    	double[] temp = solution(ic);
	    	for(int i =0;i<x.length;i++) {
	    		diff[i] = temp[i] - x[i];
	    		absdiff[i] = Math.abs(diff[i]);
	    	}
	        for(int i =0;i<x.length;i++) {
		        if (absdiff[i]>absTol) { //! first do an absolute diff
		            //! also do a rel diff if the abs diff fails (also protect for divide by zero)
		            reldiff = absdiff;
		            if(temp[i] != 0.0){
		            	reldiff[i] = absdiff[i] / Math.abs(temp[i]);}
		            }
		            if ((reldiff[i] > relTol)) {
		                System.out.println("(A)" + "Failed case");
		                System.out.println( "(//5x, a//(5x, 5d15.7))"+ "Math.expected x: "+ temp[i]);
...
//	!  the appropriate value of problem number (nProb).
		public static double[] ffcn(double[] i){
			double[] x = new double[] {10};
			return x;
}
		public static double[][] jfcn(double[] i){
...
//	        //real(wp),dimension(:),allocatable :: x
		...
		case (1):
			...
			return x;
		case (2):
			x = new double[] { -0.9999999999999953E+00, -0.1000000000000005E+01, -0.9999999999999976E+00,
					-0.9999999999999956E+00, -0.9999999999999991E+00 };
			return x;
		case (3):
			x = new double[] { -0.1677968180239693E+03, -0.8339840901198468E+02, 0.2211100430795781E+03,
					-0.4119920450599233E+02, -0.3275936360479385E+02 };
			return x;
		case (4):
			x = new double[] { -0.2029999900022674E+02, -0.9649999500113370E+01, -0.1652451975264496E+03,
					-0.4324999750056676E+01, 0.1105330585100652E+03 };
			return x;
		case (5):
			x = new double[] { 0.1000000000000000E+01, -0.2103615324224772E+03, 0.3212042081132130E+02,
					0.8113456824980642E+02, 0.1000000000000000E+01 };
			return x;
		case (6):
			x = new double[] { 0.1000000000000000E+01, 0.3321494858957815E+03, -0.4396851914289522E+03,
		...
		case (9):
			...
			return x;
		case (10):
			x = new double[] { 0.1000000000000000E+01, -0.6243301596789443E-17, 0.0000000000000000E+00 };
			return x;
		case (11):
			x = new double[] { 0.1000000000000000E+01, 0.6563910805155555E-20, 0.0000000000000000E+00 };
			return x;
		case (12):
			x = new double[] { 0.1000000000000000E+01, -0.1972152263052530E-29, 0.0000000000000000E+00 };
			return x;
		case (13):
			x = new double[] { 0.1652117596168394E-16, -0.1652117596168393E-17, 0.2643388153869468E-17,
					0.2643388153869468E-17 };
			return x;
		case (14):
			x = new double[] { 0.2016745112510229E-19, -0.2016745112510229E-20, 0.3226792180016300E-20,
					0.3226792180016300E-20 };
			return x;
		case (15):
			x = new double[] { 0.3226792180016378E-17, -0.3226792180016378E-18, 0.5162867488026213E-18,
					0.5162867488026213E-18 };
			return x;
		case (16):
			x = new double[] { 0.1141248446549937E+02, -0.8968279137315035E+00 };
			return x;
		case (17):
			x = new double[] { 0.1141300466147456E+02, -0.8967960386859591E+00 };
			return x;
		case (18):
			x = new double[] { 0.1141278178578820E+02, -0.8968051074920677E+00 };
			return x;
		case (19):
			x = new double[] { 0.8241057657583339E-01, 0.1133036653471504E+01, 0.2343694638941154E+01 };
			return x;
		case (20):
			x = new double[] { 0.8406666738183293E+00, -0.1588480332595655E+09, -0.1643786716535352E+09 };
			return x;
		case (21):
			x = new double[] { 0.8406666738676455E+00, -0.1589461672055184E+09, -0.1644649068577712E+09 };
			return x;
		case (22):
			x = new double[] { 0.1928078104762493E+00, 0.1912626533540709E+00, 0.1230528010469309E+00,
					0.1360532211505167E+00 };
			return x;
		case (23):
			x = new double[] { 0.7286754737686598E+06, -0.1407588031293926E+02, -0.3297779778419661E+08,
					-0.2057159419780170E+08 };
			return x;
		case (24):
			x = new double[] { 0.1927984063846549E+00, 0.1914736844615448E+00, 0.1230924753714115E+00,
					0.1361509629062244E+00 };
			return x;
		case (25):
			x = new double[] { 0.5609636471026654E-02, 0.6181346346286586E+04, 0.3452236346241439E+03 };
			return x;
		case (26):
			x = new double[] { 0.1291118238019656E-10, 0.3388670577625951E+05, 0.9040289597481549E+03 };
			return x;
		case (27):
			x = new double[] { -0.1572496150837821E-01, 0.1012434882329655E+01, -0.2329917223876743E+00,
					0.1260431011028187E+01, -0.1513730313944210E+01, 0.9929972729184218E+00 };
			return x;
		case (28):
			x = new double[] { -0.1572519013866764E-01, 0.1012434858601050E+01, -0.2329915458438263E+00,
					0.1260429320891618E+01, -0.1513727767065737E+01, 0.9929957342632760E+00 };
			return x;
		case (29):
			x = new double[] { -0.1572470197125875E-01, 0.1012434909256583E+01, -0.2329919227616435E+00,
					0.1260432929295550E+01, -0.1513733204527069E+01, 0.9929990192232205E+00 };
			return x;
		case (30):
			x = new double[] { -0.1530706441663782E-04, 0.9997897039345963E+00, 0.1476396349113067E-01,
					0.1463423301456550E+00, 0.1000821094549679E+01, -0.2617731120708589E+01, 0.4104403139437982E+01,
					-0.3143612262365286E+01, 0.1052626403788335E+01 };
			return x;
		case (31):
			x = new double[] { -0.1530713348497491E-04, 0.9997897039412341E+00, 0.1476396297861486E-01,
					0.1463423348188786E+00, 0.1000821073213774E+01, -0.2617731070847226E+01, 0.4104403076555856E+01,
					-0.3143612221787109E+01, 0.1052626393225985E+01 };
			return x;
		case (32):
			x = new double[] { -0.1530703652788392E-04, 0.9997897039319498E+00, 0.1476396369347192E-01,
					0.1463423283001695E+00, 0.1000821103001098E+01, -0.2617731140510708E+01, 0.4104403164468549E+01,
					-0.3143612278549893E+01, 0.1052626408008490E+01 };
			return x;
		case (33):
			x = new double[] { -0.6638060467011595E-08, 0.1000001644117862E+01, -0.5639322103223269E-03,
					0.3478205405036302E+00, -0.1567315040922755E+00, 0.1052815177186520E+01, -0.3247271153392391E+01,
					0.7288434897798658E+01, -0.1027184824113737E+02, 0.9074113646924774E+01, -0.4541375466623413E+01,
					0.1012011888539188E+01 };
			return x;
		case (34):
			x = new double[] { -0.6637102237178952E-08, 0.1000001644117874E+01, -0.5639322084840861E-03,
					0.3478205404902133E+00, -0.1567315039874129E+00, 0.1052815176715104E+01, -0.3247271152062481E+01,
					0.7288434895390663E+01, -0.1027184823833726E+02, 0.9074113644905919E+01, -0.4541375465802633E+01,
					0.1012011888395745E+01 };
			return x;
		case (35):
			x = new double[] { -0.6638060465675741E-08, 0.1000001644117862E+01, -0.5639322102727471E-03,
					0.3478205405024820E+00, -0.1567315040810628E+00, 0.1052815177127253E+01, -0.3247271153204238E+01,
					0.7288434897423372E+01, -0.1027184824066345E+02, 0.9074113646557050E+01, -0.4541375466463498E+01,
					0.1012011888509363E+01 };
			return x;
		case (36):
			x = new double[] { 0.9999999999999999E+00, 0.1000000000000000E+02, 0.1000000000000000E+01 };
			return x;
		case (37):
			x = new double[] { 0.2578199266367936E+00, 0.2578299767645596E+00 };
			return x;
		case (38):
			x = new double[] { -0.1159125521567487E+02, 0.1320248976116189E+02, -0.4035744915102742E+00,
					0.2367363118370096E+00 };
			return x;
		case (39):
			x = new double[] { -0.1159592742721295E+02, 0.1320418669262178E+02, -0.4034173628612446E+00,
					0.2367711433960093E+00 };
			return x;
		case (40):
			x = new double[] { -0.1159025975515195E+02, 0.1320206290836587E+02, -0.4036882104139269E+00,
					0.2366649366417166E+00 };
			return x;
		case (41):
			x = new double[] { 0.5000000000000000E+00 };
			return x;
		case (42):
			x = new double[] { 0.9817314924683995E+00 };
			return x;
		case (43):
			x = new double[] { 0.9817314852933997E+00 };
			return x;
		case (44):
			x = new double[] { 0.4315366485873882E-01, 0.1930916378432737E+00, 0.2663285938126948E+00,
					0.4999993346289514E+00, 0.5000006653710486E+00, 0.7336714061873052E+00, 0.8069083621567262E+00,
					0.9568463351412612E+00 };
			return x;
		case (45):
			x = new double[] { 0.4420534613578274E-01, 0.1994906723098809E+00, 0.2356191084710600E+00,
					0.4160469078925979E+00, 0.5000000000000001E+00, 0.5839530921074019E+00, 0.7643808915289401E+00,
					0.8005093276901190E+00, 0.9557946538642172E+00 };
			return x;
		case (46):
			x = new double[] { 0.5962026717536065E-01, 0.1667087838059440E+00, 0.2391710188135143E+00,
					0.3988852903458887E+00, 0.3988836678710648E+00, 0.6011163321289351E+00, 0.6011147096541113E+00,
					0.7608289811864857E+00, 0.8332912161940560E+00, 0.9403797328246394E+00 };
			return x;
		case (47):
			x = new double[] { 0.9794303033498624E+00, 0.9794303033498624E+00, 0.9794303033498624E+00,
					0.9794303033498624E+00, 0.9794303033498624E+00, 0.9794303033498624E+00, 0.9794303033498624E+00,
					0.9794303033498624E+00, 0.9794303033498624E+00, 0.1205696966501376E+01 };
			return x;
		case (48):
			x = new double[] { 0.9794303033498634E+00, 0.9794303033498634E+00, 0.9794303033498634E+00,
					0.9794303033498634E+00, 0.9794303033498634E+00, 0.9794303033498634E+00, 0.9794303033498634E+00,
					0.9794303033498634E+00, 0.9794303033498634E+00, 0.1205696966501366E+01 };
			return x;
		case (49):
			x = new double[] { 0.1000000000000002E+01, 0.1000000000000002E+01, 0.1000000000000002E+01,
					...
					0.1000000000000002E+01, 0.1000000000000002E+01, 0.9999999999999840E+00 };
			return x;
		case (50):
			x = new double[] { 0.9977542164428221E+00, 0.9977542164428221E+00, 0.9977542164428221E+00,
					0.9977542164428221E+00, 0.9977542164428221E+00, 0.9977542164428221E+00, 0.9977542164428221E+00,
					0.9977542164428221E+00, 0.9977542164428221E+00, 0.9977542164428221E+00, 0.9977542164428221E+00,
					0.9977542164428221E+00, 0.9977542164428221E+00, 0.9977542164428221E+00, 0.9977542164428221E+00,
					0.9977542164428221E+00, 0.9977542164428221E+00, 0.9977542164428221E+00, 0.9977542164428221E+00,
					0.9977542164428221E+00, 0.9977542164428221E+00, 0.9977542164428221E+00, 0.9977542164428221E+00,
		...
		case (51):
			...
			return x;
		case (52):
			x = new double[] { 0.3754100492440257E+00, 0.1935846545431133E+01, -0.1464686767487213E+01,
					0.1286753391104403E-01, 0.2212270118130737E-01 };
			return x;
		case (53):
			x = new double[] { 0.1309976638100963E+01, 0.4315524807599996E+00, 0.6336612616028594E+00,
					0.5994285609916951E+00, 0.7541797682724486E+00, 0.9043000823785187E+00, 0.1365799495210074E+01,
					0.4823731997481072E+01, 0.2398684751048711E+01, 0.4568875547914517E+01, 0.5675342062730520E+01 };
			return x;
		default:
			x = new double[] { 10 };
			return x;
...
//	                                             !! matrix of the nProb function evaluated at x.
		...
		double[] v = new double[] { 4.0, 2.0, 1.0, 5.0e-1, 2.5e-1, 1.67e-1, 1.25e-1, 1.0e-1, 8.33e-2, 7.14e-2,
				6.25e-2 };
...
//	                                        !! function evaluated at x.
		...
		double[] v = new double[] { 4.0, 2.0, 1.0, 5.0e-1, 2.5e-1, 1.67e-1, 1.25e-1, 1.0e-1, 8.33e-2, 7.14e-2,
				6.25e-2 };
		double[] y1 = new double[] { 1.4e-1, 1.8e-1, 2.2e-1, 2.5e-1, 2.9e-1, 3.2e-1, 3.5e-1, 3.9e-1, 3.7e-1, 5.8e-1,
				7.3e-1, 9.6e-1, 1.34, 2.1, 4.39 };
		double[] y2 = new double[] { 1.957e-1, 1.947e-1, 1.735e-1, 1.6e-1, 8.44e-2, 6.27e-2, 4.56e-2, 3.42e-2, 3.23e-2,
				2.35e-2, 2.46e-2 };
		double[] y3 = new double[] { 3.478e4, 2.861e4, 2.365e4, 1.963e4, 1.637e4, 1.372e4, 1.154e4, 9.744e3, 8.261e3,
				7.03e3, 6.005e3, 5.147e3, 4.427e3, 3.82e3, 3.307e3, 2.872e3 };
		double[] y4 = new double[] { 8.44e-1, 9.08e-1, 9.32e-1, 9.36e-1, 9.25e-1, 9.08e-1, 8.81e-1, 8.5e-1, 8.18e-1,
				...
				4.11e-1, 4.06e-1 };
		double[] y5 = new double[] { 1.366, 1.191, 1.112, 1.013, 9.91e-1, 8.85e-1, 8.31e-1, 8.47e-1, 7.86e-1, 7.25e-1,
				7.46e-1, 6.79e-1, 6.08e-1, 6.55e-1, 6.16e-1, 6.06e-1, 6.02e-1, 6.26e-1, 6.51e-1, 7.24e-1, 6.49e-1,
				6.49e-1, 6.94e-1, 6.44e-1, 6.24e-1, 6.61e-1, 6.12e-1, 5.58e-1, 5.33e-1, 4.95e-1, 5.0e-1, 4.23e-1,
				3.95e-1, 3.75e-1, 3.72e-1, 3.91e-1, 3.96e-1, 4.05e-1, 4.28e-1, 4.29e-1, 5.23e-1, 5.62e-1, 6.07e-1,
				6.53e-1, 6.72e-1, 7.08e-1, 6.33e-1, 6.68e-1, 6.45e-1, 6.32e-1, 5.91e-1, 5.59e-1, 5.97e-1, 6.25e-1,
				7.39e-1, 7.1e-1, 7.29e-1, 7.2e-1, 6.36e-1, 5.81e-1, 4.28e-1, 2.92e-1, 1.62e-1, 9.8e-2, 5.4e-2 };
...
//	        ! FUNCTION ROUTINE SELECTOR.
		...
		case (5):
			...
			if (x[0] > zero) {
				tmp1 = Math.atan(x[1] / x[0]) / tpi;
			}
			if (x[0] < zero) {
				tmp1 = Math.atan(x[1] / x[0]) / tpi + zp5;
			}
...
//	            ! BROWN AND DENNIS FUNCTION.
		...
		case (15):
			...
			for (i = 0; i < m; i++) {
				fVec[i] = dx * fVec[i];
				if (iev > 0)
					fVec[i] = fVec[i] + one / (dfloat(i+1) * dfloat(i+1) - one);
				iev = -iev;
			}
			break;
		case (16):
